...
def load_all_data(conn):
    """Carica tutti i dati necessari per l'analisi."""
    print("\n" + "="*80)
    ...
    print(f"✅ {len(df_tx):,} transazioni caricate")
    ...
    try:
        ...
        print(f"✅ {len(df_mempool):,} snapshot caricati")
    except:
        df_mempool = pd.DataFrame()
        print("⚠️  Nessun snapshot mempool disponibile")
...
def plot_io_distribution(df_tx):
    """Grafico distribuzione input/output."""
...
def plot_heuristics_pie(df_tx):
    """Grafico a torta per euristiche."""
...
def plot_tx_size_distribution(df_tx):
    """Distribuzione dimensione transazioni."""
...
def plot_blocks_timeline(df_tx):
    """Timeline blocchi analizzati (per visualizzare gap da pruning)."""
...
def export_tables(df_tx):
    """Esporta tabelle per LaTeX/Word."""
    print("\n⏳ Esportazione tabelle...")
...
def main():
    ...
    # Header
    print("\n" + "="*80)
    print(" " * 20 + "🔬 ANALISI COMPLETA BLOCKCHAIN - TESI")
    print("="*80)
    ...
    print("\nFile generati:")
    print("  • 5 grafici PNG (300 DPI, pronti per tesi)")
    print("  • 3 tabelle CSV (formato Excel/LaTeX compatible)")
    print("\n💡 Prossimi passi:")
    print("  • Inserisci i grafici nella tesi")
    print("  • Importa le tabelle CSV in Excel/Word/LaTeX")
    print("  • Vedi REPORT_RISULTATI_SPERIMENTALI_FINALE.md per interpretazione\n")
