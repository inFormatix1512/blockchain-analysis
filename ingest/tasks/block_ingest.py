...
class BlockIngestor:
    ...
    def ingest(self, start_height_override: int = 0, end_height_override: int = 0) -> Tuple[int, int]:
        ...
        try:
            ...
            # If a bounded range is requested (sampling), compute progress within that range.
            if start_height_override > 0 and end_height_override > 0:
                try:
                    with conn.cursor() as cur:
                        cur.execute(
                            """
                            ...
                            FROM tx_basic
                            WHERE block_height >= %s AND block_height <= %s
                            """,
                            (start_height_override, end_height_override),
                        )
                        last_height_in_range = cur.fetchone()[0] or 0
                except psycopg2.Error as exc:
                    logger.error("Error querying last processed height in range: %s", exc)
                    last_height_in_range = 0

                start_height = max(last_height_in_range + 1, start_height_override)
            elif start_height_override > 0:
                # Keep the legacy behavior: advance from what we have, but not below the override.
                start_height = max(last_height + 1, start_height_override)
            else:
            ...
            # If an explicit end height is provided, prefer the full requested range.
            if end_height_override > 0:
                end_height = min(end_height, end_height_override)
            elif self.MAX_BLOCKS_PER_RUN > 0:
                end_height = min(end_height, start_height + self.MAX_BLOCKS_PER_RUN - 1)
            
            if start_height > end_height:
                if last_height_in_range is not None:
                    logger.info(
                        "No new blocks to process in requested range (last_in_range=%d, current=%d)",
                        last_height_in_range,
                        current_height,
                    )
                else:
